...
public:
  ...
  /* Quarantine attribute taken from the source archive.  When it is set, the
     same attribute is applied to every extracted item, so that the macOS
     download-origin marking survives extraction.  This is the macOS
     counterpart of the Mark-of-the-Web handling done for Windows above. */
  CByteBuffer _mac_Quarantine;
#endif
