...
void Correct_FsPath(bool absIsAllowed, bool keepAndReplaceEmptyPrefixes, UStringVector &parts, bool isDir)
{
  ...
  for (; i < parts.Size();)
  {
    ...
    /* macOS file systems keep names in Unicode Normalization Form D.
       Archives made on Windows/Linux usually carry Form C, so extracting
       them verbatim produces names that differ byte-wise from the ones
       macOS itself creates for the same text.  Normalizing here makes the
       extracted tree byte-identical to a native one, which avoids
       look-alike duplicates in git, rsync, Node.js and on non-normalizing
       volumes such as exFAT.  Both the created directories and the file
       paths derive from these parts, so the whole tree stays consistent. */
    MacOs_NormalizeName_NFD(s);
    #endif
