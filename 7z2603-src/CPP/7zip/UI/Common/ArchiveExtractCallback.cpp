    ...
    UInt64 packSize)
{
  ...
  */
  ...
  /* Remember the quarantine attribute of the archive itself, if any.
     CloseFile() applies it to every extracted item afterwards.  Doing this
     here keeps the whole propagation independent of the archive format. */
  _mac_Quarantine.Free();
  if (_arc && !_arc->Path.IsEmpty())
    MacOs_GetXattr(us2fs(_arc->Path), MACOS_QUARANTINE_XATTR_NAME, _mac_Quarantine);
  #endif
...
#if defined(__APPLE__)
  /* Propagate the quarantine attribute of the source archive to the file we
     have just written.  macOS uses com.apple.quarantine to record where a
     file came from, and losing it on extraction would silently drop that
     marking - the same defect that was fixed for the Mark-of-the-Web on
     Windows.  Failure is deliberately not reported: FAT, exFAT and some
     network volumes cannot store extended attributes at all. */
  if (_mac_Quarantine.Size() != 0
      && !_stdOutMode
      && !_testMode
      && !_diskFilePath.IsEmpty())
  {
    MacOs_SetXattr(_diskFilePath,
        MACOS_QUARANTINE_XATTR_NAME,
        _mac_Quarantine.ConstData(),
        _mac_Quarantine.Size());
  }
#endif
